...
void main() {
    ...
    vec2 q = vec2(0.);
    q.x = fbm( st+vec2(c[0],c[1]*.03*iTime) ); // CAMBIAR A .03 el iTime
    q.y = fbm( st+vec2(c[2],c[3]) );
    vec2 r = vec2(0.);

//play with the values here!
r.x = fbm( st+ (cos(3.0*mouse.x-0.4))*q+vec2(c[5],c[6]));
r.y = fbm( st+ (6.0*mouse.y-0.5)*q*sin(-.02*iTime)+vec2(c[8]*0.05*iTime,c[9]));
float f = fbm(st+c[10]*(r+length(q) ));
color = smoothstep(vec3(0.0, 0.0, 0.0),vec3(-1.0,1.0,1.0),color);
color = mix(color,vec3(-0.976,0.2*(0.09+cos(0.2*iTime)),0.958),r.y+length(q));//   Volver todos los times a 0 Este es el verde
color = mix(color,vec3(-0.01*sin(10.1*iTime),0.0,-0.01*cos(10.13*iTime)),length(r+q));//.2+.2*(1.0+cos(0.5+.3*iTime)) //no mover los del tiempo //mover el primer time a 0.1
