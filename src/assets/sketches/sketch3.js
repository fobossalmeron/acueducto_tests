...
p.setup = function() {
  ...
  p.shader(test);//shaders are applied	
}
function initc(){
	for(var i=0;i<22;i++){
		c[i]=p.random(-5,5);
...
p.draw = function() {    
	test.setUniform("iResolution", [p.width,p.height]);//pass some values to the shader
  test.setUniform("iTime", p.millis()*.001);
